...
def demo_feature_engineering():
    ...
    logger.info("Demo: NBA Feature Engineering Pipeline")
    
    # Create sample data structure (normally you'd load real data)
    sample_data = pd.DataFrame({
        'player_id': [1, 1, 2, 2] * 10,
        'game_date': pd.date_range('2023-01-01', periods=40),
        'pts': np.random.randint(0, 40, 40),
        'reb': np.random.randint(0, 15, 40),
        'ast': np.random.randint(0, 12, 40),
        'min': ['25:30', '30:45', '28:15', '35:00'] * 10,
        'fga': np.random.randint(0, 25, 40),
        'fta': np.random.randint(0, 10, 40),
        'team_id': [1, 1, 2, 2] * 10,
        'game_home_team_id': [1, 2, 1, 2] * 10
    })
    
    # Create and use pipeline
    pipeline = create_feature_engineering_pipeline()
    engineered_data = pipeline.fit_transform(sample_data)
    
    logger.info(f"Original columns: {len(sample_data.columns)}")
    logger.info(f"Engineered columns: {len(engineered_data.columns)}")
    logger.info(f"New features: {len(engineered_data.columns) - len(sample_data.columns)}")
    
    # Validate for hypothesis testing
    validation = validate_features_for_hypothesis_testing(engineered_data)
    logger.info(f"Hypothesis testing validation: {validation}")
